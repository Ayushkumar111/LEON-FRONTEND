...
const getValidAuthToken = () => {
  ...
  try {
    ...
    if (Date.now() >= expiry) {
      // Token expired, logout user and redirect
    ...
    (set, get) => ({
      ...
      fetchCart: async () => {
        try {
          ...
          // Handle warnings from hybrid SKU system
      ...
      applyCoupon: async (couponCode) => {
        try {
          ...
          const data = await response.json();
          console.log('🎟️ Coupon API response:', response.status, data);
          ...
          if (!response.ok) {
            ...
            const errorMsg = data.message || data.error || 'Invalid coupon code';
            throw new Error(errorMsg);
          }

          // IMMEDIATE STATE UPDATE - handle different response structures
      ...
      // Fetch available coupons with eligibility check
      fetchAvailableCoupons: async (cartValue = null) => {
        try {
          set({ loadingCoupons: true });
          
          const token = getValidAuthToken();
          if (!token) {
            set({ loadingCoupons: false, availableCoupons: [] });
            return { success: false, error: 'Session expired' };
          }

          // Use cart subtotal if cartValue not provided
          const currentCart = get().cart;
          const value = cartValue ?? currentCart?.subtotal ?? 0;

          const response = await fetch(
            `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/coupons/available?cartValue=${value}`,
            {
              method: 'GET',
              headers: {
                'Authorization': `Bearer ${token}`,
          ...
          set({ 
            availableCoupons: data.data?.coupons || [],
            loadingCoupons: false 
          });

          console.log('🎟️ Available coupons fetched:', data.data);
          return { success: true, data: data.data };
        } catch (error) {
      ...
      // Get cart count for header (local state)
      getCartCount: () => {
        const cart = get().cart;
      ...
      // Fetch cart count from API (optimized for header badge)
      fetchCartCount: async () => {
        try {
          ...
          const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/cart/count`, {
            method: 'GET',
            headers: {
              'Authorization': `Bearer ${token}`,
              'Content-Type': 'application/json',
          ...
          // Update cart itemCount in state
          const currentCart = get().cart;
          if (currentCart) {
            set({ 
              cart: { ...currentCart, itemCount: data.data.count }
            });
          }

          console.log('🔢 Cart count fetched:', data.data.count);
      ...
      validateCart: async () => {
        try {
          set({ isValidating: true, validationIssues: [] });
          ...
          const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/cart/validate`, {
            method: 'POST',
            headers: {
              'Authorization': `Bearer ${token}`,
              'Content-Type': 'application/json',
          ...
          const { valid, issues, cart: validatedCart } = data.data;
          ...
          if (!valid && issues?.length > 0) {
            issues.forEach(issue => {
              ...
              } else if (issue.type === 'product_unavailable') {
                toast.error(issue.message);
              } else {
          ...
          console.log('✅ Cart validation result:', { valid, issues });
          return { success: true, valid, issues, cart: validatedCart };
        } catch (error) {
